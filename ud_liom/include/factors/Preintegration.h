#ifndef PREINTEGRATION_H_
#define PREINTEGRATION_H_
...
//        acc_w = 0.0;
//        gyr_w = 0.0;
        covariance_ = 0.0001 * Matrix<double, 15, 15>::Identity();
        g_vec_ = -Eigen::Vector3d(0, 0, 9.8);
    ...
    Matrix<double, 15, 1> evaluate(const Vector3d &Pi,
        ...
        residuals.block<3, 1>(O_P, 0) = 0.1 * (Qi.inverse() * (-0.5 * g_vec_ * sum_dt_ * sum_dt_ + Pj - Pi - Vi * sum_dt_) - corrected_delta_p);
        ...
        residuals.block<3, 1>(O_BA, 0) = Baj - Bai;//(Pj-Pimu)/(sum_dt_*sum_dt_);//Baj - Bai;
        residuals.block<3, 1>(O_BG, 0) = Bgj - Bgi;
        return residuals;
    }

    Matrix<double, 9, 1> evaluate(const Vector3d &Pi,
                                  const Quaterniond &Qi,
                                  const Vector3d &Vi,
                                  const Vector3d &Pj,
                                  const Quaterniond &Qj,
...
//        Vector3d dbg = Bgi - linearized_bg_; // NOTE: optimized one minus the linearized one
        ...
        residuals.block<3, 1>(O_P, 0) = Qi.inverse() * (-0.5 * g_vec_ * sum_dt_ * sum_dt_ + Pj - Pi - Vi * sum_dt_) - corrected_delta_p;
//        residuals.block<3, 1>(O_P, 0) = Qi.inverse() * (-0.5 * g_vec_ * sum_dt_ * sum_dt_ - Pi - Vi * sum_dt_) + Qj.inverse() * Pj - corrected_delta_p;
        residuals.block<3, 1>(O_R, 0) = 2.0 * (corrected_delta_q.inverse() * (Qi.inverse() * Qj)).normalized().vec();
        residuals.block<3, 1>(O_V, 0) = Qi.inverse() * (-g_vec_ * sum_dt_ - Vi) + Qj.inverse() * Vj - corrected_delta_v;
//        residuals.block<3, 1>(O_BA-O_R, 0) = Baj - Bai;
//        residuals.block<3, 1>(O_BG-O_R, 0) = Bgj - Bgi;
        return residuals;
    }
