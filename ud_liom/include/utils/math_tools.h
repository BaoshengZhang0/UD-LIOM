...
template <typename T>
Eigen::Quaternion<T> deltaQT(const Eigen::Matrix<T, 3, 1> &theta)
{
    Eigen::Quaternion<T> dq;
//    Eigen::Matrix<Derived, 3, 1> half_theta = theta;
//    half_theta /= static_cast<Derived>(2.0);
//    dq.w() = static_cast<Derived>(1.0);
//    dq.x() = half_theta.x();
//    dq.y() = half_theta.y();
//    dq.z() = half_theta.z();
    return dq;
