...
public:
    ...
    struct KD_TREE_NODE
    {
        PointType point;
        int division_axis;
        int TreeSize = 1; //以该点为根节点的树包含的节点数量
        int invalid_point_num = 0; //树中的失效点数量
        int down_del_num = 0;
        bool point_deleted = false; //该点是否有效
        bool tree_deleted = false; //作为根节点，对应的树是否有效
        ...
        float node_range_x[2], node_range_y[2], node_range_z[2]; //生成的树状结构中，对应的
        float radius_sq; //该点树状结构对应的空间范围
        //树结构对应的子节点和父节点
        KD_TREE_NODE *left_son_ptr = nullptr;
        KD_TREE_NODE *right_son_ptr = nullptr;
        KD_TREE_NODE *father_ptr = nullptr;
        // For paper data record
        float alpha_del; //树中标志为delete失效点的占比，判断树结构是否需要更新
        float alpha_bal; //树中单子树点云占比，判断平衡性
    };
...
private:
    ...
    float delete_criterion_param = 0.5f; //触发树节点重建参数，无效点在所有点中的比例超过该参数则开始重建树并删除这些点
    float balance_criterion_param = 0.7f; //树平衡操作参数，一旦左子树点数占比超过该采数则进行树平衡操作
    float downsample_size = 0.2f; //降采样体素网格大小
